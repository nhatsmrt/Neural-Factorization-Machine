...
class FMClassifier():
    def __init__(self, inp_dim, emb_dim = 3, n_classes=47, keep_prob=0.8, use_gpu=False):
    ...
    def create_network(self, inp_dim, emb_dim):
        ...
        self._op = self._bipooled + tf.matmul(self._X_norm, self._W) + self._global_bias
    ...
    def bi_interaction_layers(self, embedding):
        return 0.5 * ( tf.reduce_sum(embedding, axis = 1) * tf.reduce_sum(embedding, axis = 1) - tf.reduce_sum(embedding * embedding, axis = 1))
    # Train:
